...
class Inception3(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        localization_map_normed  = self.get_atten_map(out1, label, True)
        ...
        if self.training == False:
            localization_erasemap_normed = self.get_atten_map(out_erase,label,True)
            htmp = torch.max(localization_map_normed,localization_erasemap_normed)
            return htmp, [logits_1,]
