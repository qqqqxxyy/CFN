...
class ResNet(nn.Module):
    ...
    def forward(self, x, label=None): 
        ...
        localization_map_normed  = self.get_atten_map(out, label, True) # (b,200,28,28) -> (20,28,28)
        self.attention = localization_map_normed    #(b,28,28)        
        
        feat_resize = self.feat_select_4slide(feat)
        self.cut_heatmap=[]
        for i in range(4):
            heatmp = self.cls_erase(feat_resize[i]) #feat_resize[i] : (b,512,28,28)
            #heatmp (b,200,28,28)
            self.cut_heatmap.append(heatmp)
            feat_resotred = self.restore4(heatmp , i)#(b,200,28,28)
            if i == 0 :
    ...
    def feat_select_4slide(self , feat):
        '''input : feat : (b,512,28,28)
           output:feat_resize_batch(4,b,512,28,28)
       '''
        for i in range(feat.shape[0]):
            feat_resize = self.cal_4slide(feat[i]) #(4,512,28,28)
            feat_resize = feat_resize.unsqueeze(1)
            if i == 0 :
                feat_resize_batch = feat_resize
            else : 
                feat_resize_batch = torch.cat((feat_resize_batch , feat_resize) , dim = 1)
        feat_resize_batch = Variable( feat_resize_batch.cuda() ) #(4,b,512,28,28)
        return feat_resize_batch

    def cal_4slide(self , feature_map):
    ...
    def cal_4idx(self , i):
        ...
        if i == 1 :
            y0,x0,y1,x1 = (0,7,20,27)
        if i == 2 :
            y0,x0,y1,x1 = (7,0,27,20)
        if i == 3 :
            y0,x0,y1,x1 = (7,7,27,27)
        
        return x0 , y0 , x1 , y1

    def restore4(self , heatmp , i):
        '''put the 4 slides retored to its original position
        '''
    ...
    def get_localization_maps(self):
        ...
        information_dict = {'map_erase':map_erase,'map1':map1,'cut_heatmp':cut_heatmp,\
            'feat' : self.feat }
