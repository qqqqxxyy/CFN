...
from data import OriDataset,ObjLocDataset,Trans,TransBbox,ObjMaskDataset,TransMask,Random_noise
from data import CombData_pre as CombData
...
def main():
    ...
    TrainOri = ObjMaskDataset(param.image_root_dir,param.mask_root_dir,param.image_property_dir,
                         tsfm=transforms.Compose([TransMask(['Resize',(156,156)]), 
                                    TransMask(['RandomCrop',(128)]), 
    ...
    for i in param.phase_lis:
        ...
        TrainLoader =  DataLoader(TrainOri,batch_size=param.batch_size,shuffle=True,\
                pin_memory=True,num_workers=6)
        synthetic_score = train_segmentation(
            model,pretrain_info,TrainLoader, param, info_record, param.gen_devices)
...
def train_segmentation(model,pretrain_info,CombLoader,params,info_record,gen_devices):
    ...
    while True:
        ...
        for _, dat in enumerate(CombLoader):
            ...
            logits = model(image)
            loss = model.module.get_loss(logits,mask)

            model.module.loss_count(loss[0])
