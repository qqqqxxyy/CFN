...
class MaskGenerator(nn.Module):
    def __init__(self, G, bg_direction, params,
                 ...
                 zs=None, zs_cls = None, z_noise=0.0):
    ...
    def gen_samples(self, z=None,batch_size=None):
        ...
        if z is None:
            z ,_= self.make_noise(batch_size)
        img = self.G(z)
    ...
    @torch.no_grad()
    def filter_by_area(self, img_batch, img_pos_batch, ref_batch,index_batch):
        '''这里的mask是每个index是否保留的bool指标
        '''
        if self.p.mask_size_up < 1.0:
            ...
            if torch.all(~mask):
                return None
            img_batch, img_pos_batch, ref_batch,index_batch = \
                img_batch[mask], img_pos_batch[mask], ref_batch[mask],index_batch[mask]
        return img_batch, img_pos_batch, ref_batch,index_batch

    @torch.no_grad()
    def filter_by_maxes_count(self, img_batch, img_pos_batch, ref_batch,index_batch):
        '''这里的mask是每个index是否保留的bool指标
        '''
    ...
    def forward(self, max_retries=100, z=None, return_steps=False):
        ...
        while img is None or img.shape[0] < self.p.batch_size:
            ...
            if z is None or step > 1:
                z ,index_batch= self.make_noise(self.p.batch_size)

            img_batch, img_pos_batch, ref_batch = \
                self.gen_samples(z=z)

            # filtration
            mask_area_filtration = self.filter_by_area(img_batch, img_pos_batch, ref_batch,index_batch)
            if mask_area_filtration is not None:
                img_batch, img_pos_batch, ref_batch ,index_batch= mask_area_filtration
            else:
                continue
            ...
            if maxes_count_filtration is not None:
                img_batch, img_pos_batch, ref_batch ,index_batch= maxes_count_filtration
            else:
                continue
            ...
            #这里生成的图片都是(-1,1) 分布的
            if img is None:
                img, ref,index = img_batch, ref_batch,index_batch
            else:
                img = torch.cat([img, img_batch])[:self.p.batch_size]
                ref = torch.cat([ref, ref_batch])[:self.p.batch_size]
                index = torch.cat([index, index_batch])[:self.p.batch_size]

        if self.p.syn_norm==True:
            img = (img.cpu()+1)/2 #虽然totensor都会进行归一化，但只有先转成(0,1)再操作才是正常的
            ...
            for i in range(img.shape[0]):
                tmp = tsfm(img[i,:])
                if tol_img == None:
                    tol_img = tmp.unsqueeze(0)
                else:
                    tol_img = torch.cat([tol_img,tmp.unsqueeze(0)])
            img = tol_img.cuda()
...
def it_mask_gen(mask_gen, devices, out_device='cpu', delete_orig=True):
    ...
    while True:
        ...
        yield img.to(out_device), ref.to(out_device), index.to(out_device)
